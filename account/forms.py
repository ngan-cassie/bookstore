...
class RegistrationForm(forms.ModelForm):
    ...
    email = forms.EmailField(max_length=100, help_text="Required", error_messages={'required': 'Sorry. You need to enter an email!'})
    ...
    def clean_username(self):
        ...
        if r.count(): # count = 1 means found out the value 
...
class UserEditForm(forms.ModelForm):
    email = forms.EmailField(label="Account email (cannot be changed)", max_length=200, widget=forms.TextInput(
        attrs={'class':'form-control mb-3', 'placeholder':'Firstname','id':'form-email', 'readonly':'readyonly'}
    ))
    ...
    class Meta:
        model = UserBase
        fields = {'email', 'first_name', 'user_name'}
